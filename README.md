...
애플리케이션 빌드는 Java/Maven 설치 없이 Maven 컨테이너에서 순차 실행됩니다. 1GB RAM 서버이므로 두 Java 컨테이너의 힙을 각각 256MB로 제한했습니다. 빌드 중 메모리가 부족하면 임시 swap을 추가하거나, CI에서 이미지를 빌드해 서버에서는 pull만 하는 방식을 권장합니다.
